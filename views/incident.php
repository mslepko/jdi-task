...
if($_POST['submitNewIncident']):
	...
	if(empty($error)):
		...
		$Report = new Reports;
		if(!empty($post['idReport'])):
			if($Report->updateReport($post)):
				$info = 'Report updated';
				unset($post);
			else:
				$info = 'Report not updated. Try again later.';
			endif;
		else:
			if($Report->addReport($post)):
				$info = 'Report added';
				unset($post);
			else:
...
if(isset($_GET['id_report']) && empty($post)):
...
<fieldset>
	<div class="row">
		...
		<input type="text" id="incident_date" name="incident_date" value="<?=isset($post['incident_date'])?date('Y/m/d H:i',$post['incident_date']):'YYYY/mm/dd HH:mm'?>" onblur="javascript:if(this.value=='') {this.value='YYYY/mm/dd HH:mm';}" onfocus="javascript:if(this.value=='YYYY/mm/dd HH:mm') {this.value='';}"/>
	</div>
	<div class="row">
		<label for="resolution_date">Date / Time of Resolution:</label>
		<input type="text" name="resolution_date" id="resolution_date" value="<?=isset($post['resolution_date'])?date('Y/m/d H:i',$post['resolution_date']):'YYYY/mm/dd HH:mm'?>" onblur="javascript:if(this.value=='') {this.value='YYYY/mm/dd HH:mm';}" onfocus="javascript:if(this.value=='YYYY/mm/dd HH:mm') {this.value='';}"/>
	</div>
	<div class="row">
	...
	<div class="row">
		<?=(isset($_GET['id_report']) && $_GET['id_report']>0)?'<input type="hidden" name="idReport" value="'.$post['idReport'].'"/>':''?>
